...
#if USE_ATOMIC_TYPES
  _Atomic(Cell_t*) VOLATILE pNext __attribute__ (( aligned (64) )); // Next message
#else
  Cell_t* pNext __attribute__ (( aligned (64) )); // Next message
#endif
  Msg_t* pMsg;
} Cell_t;
...
#if USE_ATOMIC_TYPES
  _Atomic(Cell_t*) VOLATILE pHead __attribute__(( aligned (64) ));
