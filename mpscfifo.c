...
void add(MpscFifo_t *pQ, Msg_t *pMsg) {
  ...
  pCell->pNext = NULL;
  pCell->pMsg = pMsg;
  Cell_t* pPrev = __atomic_exchange_n((Cell_t**)&pQ->pHead, pCell, __ATOMIC_ACQ_REL);
...
Msg_t *rmv_non_stalling(MpscFifo_t *pQ) {
  ...
  if (pNext != NULL) {
    ...
    pMsg->pCell = pTail;
    pQ->pTail = pNext;
    DPF(LDR "rmv_non_stalling:1-pQ=%p msg=%p pool=%p arg1=%lu arg2=%lu\n", ldr(), pQ, pMsg, pMsg->pPool, pMsg->arg1, pMsg->arg2);
    pQ->msgs_processed += 1;
    return pMsg;
  } else {
...
Msg_t *rmv(MpscFifo_t *pQ) {
  ...
  if ((pNext == NULL) && (pTail == pQ->pHead)) {
    ...
    if (pNext == NULL) {
      ...
      DPF(LDR "rmv:3 stalling %u times pQ=%p count=%d pNext=%p\n", ldr(), i, pQ, pQ->count, pNext);
    }
    Msg_t* pMsg = pNext->pMsg;
    pMsg->pCell = pTail;
    pQ->pTail = pNext;
    ...
    DPF(LDR "rmv:4-pQ=%p msg=%p pool=%p arg1=%lu arg2=%lu\n", ldr(), pQ, pMsg, pMsg->pPool, pMsg->arg1, pMsg->arg2);
    return pMsg;
  }
}
